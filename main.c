...
uint32_t lastDebounceTime = 0;  // To track debounce timing
uint32_t debounceDelay = 200;   // Debounce delay in milliseconds
uint8_t toggleFreq = 2; // Start with 2 Hz frequency
uint32_t adc_val = 0;//variable to store ADC value
uint32_t CCR = 0; //variable to store CCR value
uint8_t binary_values[] = {
...
uint32_t TIM6Period = 250;//TIM6 period to 250ms
char lcd_buffer[16]; // Buffer for storing the string to be displayed on the LCD
static uint16_t eeprom_address = 0; // Counter to keep track of EEPROM address
...
int main(void)
{
  ...
  for (uint16_t i = 0; i < sizeof(binary_values); i++) {
      write_to_address(i, binary_values[i]);
  }
  ...
  while (1)
  {
	...
	// TODO: Poll ADC
	  // Poll ADC and adjust brightness of LED PB0
    adc_val = pollADC(); // Call the function to get the ADC value

	// TODO: Get CRR
    CCR = ADCtoCCR(adc_val); // Get CCR
...
void EXTI0_1_IRQHandler(void)
{
	...
	// Debounce: Check if the required debounce delay has passed
	    if ((HAL_GetTick() - lastDebounceTime) > debounceDelay)
	    {
	        // Toggle frequency between 1 Hz and 2 Hz
	        // Delay based on current frequency: 500 ms for 1 Hz, 250 ms for 2 Hz
	                if (toggleFreq == 1) {
	                	toggleFreq = 2;
	                	TIM6Period=250; // 1 Hz blinking
	                } else {
	                	toggleFreq=1; //
	                	TIM6Period=500; // 2 Hz blinking
	                }
...
void TIM16_IRQHandler(void)
{
	...
	// Read byte from EEPROM
	    uint8_t X = read_from_address(eeprom_address);

	    // Check if the read value matches the expected value
	    uint8_t expected_value = binary_values[eeprom_address];
	    if (X != expected_value) {
	        // Update LCD to show error message
	        snprintf(lcd_buffer, sizeof(lcd_buffer), "SPI ERROR!");
	        writeLCD(lcd_buffer);

	        HAL_GPIO_WritePin(GPIOB, LED7_Pin, GPIO_PIN_SET); // Turn on LED7 for error indication
	    } else {
	        // Update LCD to show the correct value
	        snprintf(lcd_buffer, sizeof(lcd_buffer), "%d", X);
	        writeLCD(lcd_buffer);

	        // Turn off LED7 if the EEPROM read was correct
	        HAL_GPIO_WritePin(GPIOB, LED7_Pin, GPIO_PIN_RESET);
	    }

	    // Update the EEPROM address for the next read
	    eeprom_address++;
	    if (eeprom_address >= sizeof(binary_values)) {
	        eeprom_address = 0;  // Wrap around to the start of the array
	    }
	// TODO: Change LED pattern; output 0x01 if the read SPI data is incorrect
	    __HAL_TIM_SET_AUTORELOAD(&htim6, TIM6Period - 1); // Set TIM6 period to 250ms for 2Hz
...
void writeLCD(char *char_in){
  ...
  lcd_command(0xC0);
  lcd_putstring(char_in);// Send string to LCD
...
uint32_t pollADC(void){
	...
	val = HAL_ADC_GetValue(&hadc); }// get the adc value
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR value (val) using an appropriate equation
	// Scale the 12-bit ADC value (0-4095) to match the ARR range (0-47999)
